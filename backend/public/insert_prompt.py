...
def ensure_prompt_exists():
    db = SessionLocal()
    try:
        prompt_existente = db.query(Prompt).filter_by(opcion="generacion_archivos_docker").first()
        if not prompt_existente:
            ...
            db.add(nuevo_prompt)
            db.commit()
            logger.info("Prompt 'generacion_archivos_docker' insertado.")
        else:
            logger.info("Prompt 'generacion_archivos_docker' ya existe, no se insertó.")
    except Exception as e:
        logger.error(f"Error asegurando prompt: {e}")
    finally:
        db.close()
